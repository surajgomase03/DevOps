...
- **2026-07-30 — Batch 1 (40 interview questions, basic/intermediate/advanced):** 34 duplicates skipped, 5 new concepts added (§4.2 docker run vs docker start, §9.2 sharing data between containers, §10.4 scaling with Compose, new "Docker Context" section in Phase 2, §8.3 --link vs bridge vs overlay in Phase 7), 1 existing section extended (§11.2b Docker-native secrets merged into Phase 11). Full summary at end of this changelog block for this date.
...
- **What it is:** A named, saved connection endpoint (socket/host + TLS config) that tells the `docker` CLI which Docker daemon to talk to. The Docker client is decoupled from the daemon (see Client–Server Architecture above) — context is how you point the same CLI at different daemons without retyping `-H`/`DOCKER_HOST` every time.
- **Why it matters:** One engineer's laptop CLI can target local Docker Desktop, a remote EC2 Docker host, and a CI runner's daemon — by switching context, not reinstalling anything.

| Command | Purpose |
|---|---|
| `docker context ls` | List all saved contexts (default marked with `*`) |
| `docker context create remote-ec2 --docker "host=ssh://ec2-user@10.0.1.5"` | Create a context pointing at a remote daemon over SSH |
| `docker context use remote-ec2` | Switch the active context — all subsequent `docker` commands target that host |
| `docker context inspect remote-ec2` | Show full endpoint/TLS config for a context |
| `docker context rm remote-ec2` | Delete a saved context |

> **🟢 CMG Project Example:** Used during EC2-hosted legacy app work (Siebel/WebSphere containers) — switching context to the target EC2 host lets you run `docker ps`/`docker logs` against that host's daemon directly from a local terminal, instead of SSHing in first via SSM Session Manager.
>
> **🎯 Interview Tip:** This question is really testing whether you understand Docker's client-server split. If you know the daemon can live anywhere reachable over a socket, `docker context` is just "which socket am I talking to" — an easy follow-on from the architecture question.
...
#### 4.2 docker run vs docker start

| | **docker run** | **docker start** |
|---|---|---|
| Purpose | Create a NEW container from an image AND start it | Start an EXISTING (stopped) container |
| Image required | Yes — pulls/uses an image | No — container already exists from a prior `docker run`/`docker create` |
| Config (ports/volumes/env) | Set at creation time via flags (`-p`, `-v`, `-e`) | Cannot change — reuses the config from when the container was created |
| Typical use | First time launching a workload | Restarting a stopped container without recreating it |
| Equivalent to | `docker create` + `docker start` | — |

> **🎯 Interview Tip:** Interviewers use this to check if you understand container *identity*. A container is not the same as an image — `docker run` makes a new container instance every time; `docker start` reuses one that already exists, with its original config frozen in.
...
#### 8.3 --link vs User-Defined Bridge vs Overlay Networks

| | **--link** | **User-Defined Bridge** | **Overlay** |
|---|---|---|---|
| Status | ❌ Deprecated legacy flag | ✅ Current single-host standard | ✅ Current multi-host standard |
| Scope | Single host, single link between 2 containers | Single host, any number of containers | Multiple hosts (Swarm cluster) |
| DNS resolution | Manual `/etc/hosts` injection — fragile | Automatic, by container/service name | Automatic, by service name across nodes |
| Direction | One-way (source container's env vars/hosts exposed to target only) | Bidirectional — any container can reach any other on the network | Bidirectional across the whole cluster |
| Config on restart | Breaks if linked container is recreated (IP/env stale) | Survives container recreation — DNS re-resolves | Survives container recreation across nodes |
| Recommendation | 🚫 Never use in new work | ✅ Default choice for standalone Docker/Compose | ✅ Use for Swarm; EKS uses VPC CNI instead (see Phase 16) |

> **❌ Common Trap:** Don't mention `--link` as a current best practice — it signals outdated knowledge. Correct interview answer: "`--link` is deprecated; user-defined bridge networks give the same container-to-container communication with proper DNS and no direction restriction, which is why Docker recommends bridge networks for single-host and overlay for multi-host by default."
...
- **Named volume, mounted in multiple containers (recommended):**
  `docker run -v shared-data:/data app1` + `docker run -v shared-data:/data app2` — both containers read/write the same volume concurrently.
- **`--volumes-from` (legacy, mostly Compose-era pre-cursor):**
  `docker run --volumes-from container1 app2` — mounts all of `container1`'s volumes into `app2`. Rarely used in modern Docker; superseded by named volumes declared once in Compose/Helm and referenced by multiple services/pods.
- **In CMG:** shared config/cache between sidecar containers in the same EKS pod uses a Kubernetes `emptyDir` volume (pod-level, not `--volumes-from`) — `--volumes-from` is Docker-CLI-only and doesn't translate to Kubernetes.

> **❌ Common Trap:** Don't confuse this with networking — sharing *data* uses volumes; sharing *network namespace* (e.g. `docker run --network container:x`) is a different, unrelated mechanism.
...
#### 10.4 Scaling Services with Compose

- `docker-compose up -d --scale web=3` — runs 3 replicas of the `web` service. Requires the service to **not** have a fixed `container_name` (names must be auto-generated: `web_1`, `web_2`, `web_3`) and **not** map a fixed host port (e.g. use `ports: - "8080"` not `"8080:8080"`, or omit host port mapping and put a load balancer/reverse proxy in front).
- Compose has no built-in load balancer — for real traffic distribution across scaled replicas you need an nginx/HAProxy service in the same compose file, or move to Swarm/Kubernetes which handle this natively.
- **In CMG:** Compose `--scale` is dev/local-only for quick multi-instance testing. Actual scaling in production is via Kubernetes HPA (Horizontal Pod Autoscaler) on EKS — see Phase 12 Monitoring for the metrics HPA scales on.

> **🎯 Interview Tip:** If asked "how do you scale in Docker," don't just say `--scale` — an experienced answer distinguishes Compose `--scale` (dev-only, no load balancing) from Swarm `docker service scale` (built-in routing mesh) from Kubernetes HPA (production, metric-driven autoscaling).
...
#### 11.2b Docker-Native Secret Mechanisms (Compose & Swarm)

*📌 Ref: §24.4 for BuildKit secret mounts (build-time) and §11.3 below for how CMG actually manages runtime secrets on EKS. This subsection fills the gap on Docker's own built-in secrets primitives, for completeness.*

| Mechanism | Scope | How it works |
|---|---|---|
| `docker secret create` | Swarm only | Encrypts secret at rest in the Swarm raft log; mounted as a tmpfs file (`/run/secrets/<name>`) inside the container — never as an env var |
| Compose `secrets:` block | Docker Compose (local/dev) | References a file or external Swarm secret; mounted read-only into the container filesystem, not injected as env var |
| Plain `docker run -e` | Any | ⚠️ Anti-pattern for real secrets — visible in `docker inspect`, shell history, and process list |

> **✅ Correct Answer:** Docker's own secret primitives (`docker secret`, Compose `secrets:`) exist but are Swarm-scoped. **In CMG we don't use them** — we're on EKS, not Swarm, so runtime secrets go through Kubernetes Secrets (or Vault, for higher-sensitivity values) as already covered in §11.3 below. Know both exist, but be clear about which one applies to your actual orchestrator.
