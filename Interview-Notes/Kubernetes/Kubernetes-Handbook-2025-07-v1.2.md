...
# UPDATES — MERGED FROM UPLOADED FILES (July 2025)

> All content inserted verbatim per Master Prompt Rule 0 (paste = insert as-is).
> Placed under most relevant topic. Duplicate check run against S01–S50 and Interview Q&A.

---

## S06 — NETWORKING — UPDATES (Extends existing S06)

### kube-proxy Internals (Verbatim from add.txt)

- kube-proxy runs on every node and watches the API server for Service/Endpoint changes.
- It programs iptables (or IPVS, more scalable) rules on each node so traffic to the Service's ClusterIP gets load-balanced across the current healthy Pod IPs — no central load balancer required for internal traffic.

### CNI Deep Dive (Verbatim from add.txt)

CNI is the standard interface/spec that Kubernetes uses to configure networking for Pods — assigning IPs, setting up routes, and enforcing network policy — implemented by a pluggable CNI plugin.

**How It Works:**
- When kubelet creates a Pod, it calls the configured CNI plugin
- CNI plugin allocates an IP from its pool
- Wires up the Pod's network namespace
- Attaches it to the cluster network so it can reach other Pods/Services

### Troubleshooting: Pods Stuck in ContainerCreating Due to IP Exhaustion

> **[NEW TOPIC — from add.txt — not covered in S06]**

**Scenario:** Pods remain in ContainerCreating state with FailedCreatePodSandBox events.

**Step-by-Step Diagnosis:**

```bash
# Step 1: Describe the Pod — look at Events
...
# Step 3: Check CNI Plugin
kubectl get pods -n kube-system
# EKS: aws-node | Calico: calico-node | Flannel: kube-flannel
kubectl logs aws-node-xxxxx -n kube-system
# Look for: Failed to allocate ENI IP / No available IP addresses

# Step 4: Check Pod Density on node
kubectl get pods -A -o wide
kubectl describe node worker-1
# Look for: Non-terminated Pods count

# Step 5: Verify Subnet Capacity (AWS)
# 10.0.1.0/24 = only 251 usable IPs
# If subnet exhausted → no new pods

# Step 6: Check CNI Configuration
kubectl get daemonset aws-node -n kube-system
# Verify: WARM_IP_TARGET and MINIMUM_IP_TARGET

# Step 7: Verify kubelet Max Pods
cat /etc/kubernetes/kubelet/kubelet-config.json
# or: ps -ef | grep max-pods
# --max-pods=110
```
...
| Cause | Symptom |
|---|---|
| Subnet IP exhausted | No IP available |
| ENI limit reached (AWS) | Cannot assign Pod IP |
| Too many pods on node | Pod stuck in ContainerCreating |
| CNI plugin failure | Sandbox creation fails |
| CNI DaemonSet crashed | Network setup fails |

**AWS Instance IP Limits Example:**
- t3.medium: Max ENIs=3, IPs per ENI=6 → Maximum Pods ≈ 17
- If all IPs consumed → Pod stuck in ContainerCreating

**How to Fix:**

```bash
# Option 1: Add more worker nodes (Cluster Autoscaler)

# Option 2: Expand subnet (/24 → /20 = more IPs)

# Option 3: Enable Prefix Delegation (AWS — many more IPs per ENI)

# Option 4: Use larger EC2 instance (t3.medium → m5.large)

# Option 5: Restart CNI if unhealthy
kubectl rollout restart daemonset aws-node -n kube-system

# Option 6: Remove unused Pods
kubectl delete pod <pod-name>
kubectl scale deployment app --replicas=2
```

**Interview Answer (2-minute version):**
> "If Pods are stuck in ContainerCreating, I first run kubectl describe pod and check the Events section. If I see errors such as 'failed to assign an IP address' or 'failed to create pod sandbox', I suspect IP exhaustion or a CNI networking issue. Next, I verify that the node is Ready with kubectl get nodes, inspect the CNI plugin (such as aws-node in EKS) and its logs, and check pod density on the node with kubectl describe node. In cloud environments like AWS, I also verify whether the subnet has available IP addresses and whether the instance has reached its ENI or pod limits. Depending on the root cause, I would add worker nodes, expand the subnet, enable prefix delegation, use larger instance types, or fix the CNI plugin if it's unhealthy."

---

### Service Not Routing Traffic — Generalized Troubleshooting (from add.txt)

> **[EXTENDING S48 Scenario 2 — adding generalized framework]**

- Confirm the Service's selector actually matches the Pods' labels — a typo or a label change during a deployment is the single most common root cause.
- Check the Endpoints object — if it's empty despite Pods running, the selector mismatch or readiness failure is confirmed.
- Check Pod readiness specifically, **not just Running status** — Running ≠ Ready.
- Check the Service's `targetPort` matches the container's actual listening port — mismatch (e.g. Service says 8080 but app listens on 8081) causes connection refused even with everything else correct.

### 502/503 Errors After Deploy (from add.txt)

> **[NEW — not in existing sections]**

- **502 (Bad Gateway):** ALB/Ingress reached a target but got invalid response — often a Pod that's running but not ready (missing/misconfigured readinessProbe).
- **503 (Service Unavailable):** ALB/Ingress couldn't find any healthy target at all — check target group health directly.

**Root Causes:**
- Deployment's readinessProbe not properly gating traffic
- `maxUnavailable`/`maxSurge` too aggressive for how long app takes to become ready
- Old Pods terminated before in-flight connections finish (missing `preStop` hook or `terminationGracePeriodSeconds` too short)

**Isolation Steps:**
```bash
# Check ALB target group health (Kubernetes-layer or app-layer?)
# Check kubectl get endpoints alongside target group registration
# Check application logs/readiness endpoint directly
kubectl get endpoints <svc-name> -n <namespace>
kubectl logs <pod> --previous
```

### Pod Stuck in Pending — Causes (from add.txt)

> **[EXTENDING S48 — adding structured cause list]**

- `kubectl describe pod` on a Pending Pod shows scheduler events explaining exactly why — always the FIRST command.
- **Common causes:**
  - Resource requests too high for any node's allocatable capacity
  - No node matching a nodeSelector/affinity rule
  - Taint on all available nodes with no matching toleration
  - PVC that can't be bound (e.g. requesting specific AZ's EBS when no node in that AZ)
  - **EKS specifically:** "Insufficient pods" (not CPU/memory) points at ENI/IP exhaustion

---

## S05 — SERVICES — UPDATES (Extends existing S05)

### CoreDNS Service DNS — Verbatim (from add.txt)

Kubernetes' internal DNS (CoreDNS) provides a fully qualified name for every Service: `<service-name>.<namespace>.svc.cluster.local`, which resolves cluster-wide regardless of which namespace the caller is in.

**How It Works:**
- Within the same namespace, Pods can just use the short Service name (e.g. `backend-svc`) — DNS search domains fill in the rest.
- Across namespaces, the short name alone won't resolve correctly (it would search the caller's own namespace first) — you must qualify it with the target namespace.

### Node Not Receiving Traffic — Troubleshooting (from add.txt)

```bash
# Step 1: Node health
kubectl get nodes -o wide                              # NotReady? SchedulingDisabled?
kubectl describe node <node-name>                      # taints, conditions, resource pressure

# Step 2: Pod status on that node
kubectl get pods -o wide --field-selector spec.nodeName=<node-name>
kubectl get pods -o wide -n cmg-app | grep <node-name>  # check READY column, not just STATUS

# Step 3: Endpoints — is this node's Pod IP even listed?
kubectl get endpoints cmg-app-svc -n cmg-app -o yaml

# Step 4: Node networking
kubectl get pods -n kube-system -o wide | grep <node-name>   # check kube-proxy / aws-node
kubectl logs <pod-on-that-node> --previous

# Step 5: Remediate
# restart CNI/kube-proxy pod, cordon+drain if unhealthy, fix readiness probe/app
```

**Triage framework:**
- Step 1: Node health → check Ready, resource pressure, taints
- Step 2: Pod status → Running ≠ Ready (readiness ≠ running)
- Step 3: Endpoints → if Pod Running but not Ready, excluded from Service entirely
- Step 4: Node networking → crashed aws-node or kube-proxy = stale iptables rules
- Step 5: Logs and events → then remediate

### Service Mistakes from Production (from task3-mistakes-and-concepts.md — verbatim)

#### MISTAKE 1 — Wrong selector on Service (too broad)

```yaml
# ❌ What I did wrong
selector:
  env: dev        # matches ANY pod with this label
  team: payments  # matches ANY pod with this label

# ✅ What is correct
selector:
  app.kubernetes.io/name: payment-api   # specific to this app
  env: dev                              # scoped to environment
```

**Why it matters:**
- A Service selector matches **all pods** in the namespace with those labels
- Too broad = traffic accidentally goes to unrelated pods (fraud-detection, audit-service, etc.)
- Zero error shown — traffic just silently routes to wrong pods
- Always use `app.kubernetes.io/name` as the primary selector — it's app-specific

**Interview answer:** *"If the Service selector is too broad, it will include unintended pods as endpoints. This causes silent misrouting — traffic reaches wrong pods with no logs or errors. Always scope the selector to the exact app using `app.kubernetes.io/name`."*

---

#### MISTAKE 2 — Wrong NLB annotations on EKS LoadBalancer Service

```yaml
# ❌ Wrong
annotations:
  service.beta.kubernetes.io/aws-load-balancer-type: "external"       # provisions Classic ELB
  service.beta.kubernetes.io/aws-load-balancer-nlb-target-type: "ip"  # wrong annotation

# ✅ Correct
annotations:
  service.beta.kubernetes.io/aws-load-balancer-type: "nlb"            # provisions NLB
  service.beta.kubernetes.io/aws-load-balancer-internal: "true"       # keeps inside VPC
```

| Wrong annotation | Result |
|---|---|
| `type: "external"` | Provisions Classic ELB (deprecated, worse performance) |
| Missing `internal: "true"` | NLB becomes internet-facing — payment API exposed publicly |
| Wrong target type | Traffic routing breaks silently |

**Rules:**
- `type: "nlb"` → provisions Network Load Balancer (correct for production EKS)
- `internal: "true"` → keeps NLB inside VPC (critical for internal services)
- Without `internal: "true"`, your service is **publicly accessible by default** in AWS
- Classic ELB is deprecated — never use it for new workloads

---

#### MISTAKE 3 — sessionAffinity: None with sessionAffinityConfig block present

```yaml
# ❌ Contradictory
sessionAffinity: None        # says None
sessionAffinityConfig:       # config block for something set to None
  clientIP:
    timeoutSeconds: 10800

# ✅ Correct
sessionAffinity: None        # clean — no config block needed
```

- `sessionAffinityConfig` is only valid when `sessionAffinity: ClientIP`
- Having both `None` + `sessionAffinityConfig` is a contradiction — Kubernetes ignores config block

---

#### MISTAKE 4 — sessionAffinity: ClientIP on a stateless payment API

```yaml
# ❌ Wrong for stateless service
...
**Why wrong for stateless:**
- ClientIP = all requests from one client go to same pod for timeout duration
- On 3 replicas: each client pins to 1 pod → 2 pods receive zero traffic from that client
- If pinned pod crashes → client gets errors until timeout expires (up to 3 hours)

**When to use `sessionAffinity: ClientIP`:**
- WebSocket connections (must stay on same pod)
- Session-based authentication (session stored in pod memory, not Redis)
- Legacy apps that don't support distributed state

**When to use `sessionAffinity: None` (default):**
- REST APIs (stateless) — microservices, anything storing state in DB/cache

**Interview answer:** *"SessionAffinity: ClientIP pins a client to one pod for a duration. For stateless APIs this reduces load balancing effectiveness and creates a single point of failure per client. Use None for stateless services so kube-proxy distributes requests evenly across all healthy pods."*

---

#### MISTAKE 5 — Verifying old resources instead of newly applied ones
...
# 2. Verify AFTER
kubectl describe svc payment-api-internal -n finpay-system
kubectl describe svc payment-api-external -n finpay-system
```

> In production, verifying the wrong version of a resource and declaring it healthy = missed incident.

---

#### MISTAKE 6 — Inline copy-paste of review comments into YAML

```yaml
# ❌ Wrong — copied reviewer feedback text as comments
selector:
  app.kubernetes.io/name: payment-api   # ✅ specific to this app
  env: dev                               # ✅ environment scoping

# ✅ Correct — write your OWN comments explaining WHY
selector:
  app.kubernetes.io/name: payment-api   # scope traffic to this specific app only
  env: dev
```

> Wrong comments (explaining opposite behavior) are **worse than no comments**.

---

### Service Core Concepts (from task3-mistakes-and-concepts.md — verbatim)

#### Three Service types

| Type | Use case | Creates AWS resource? |
|---|---|---|
| `ClusterIP` | Internal pod-to-pod traffic only | No |
| `NodePort` | Dev/testing, exposes on node IP | No |
| `LoadBalancer` | External traffic entry point | Yes — NLB/ALB (costs money) |

#### port vs targetPort vs nodePort

| Field | What it is |
|---|---|
| `port` | Port the Service exposes (what clients call) |
| `targetPort` | Port on the pod/container (what the app listens on) |
| `nodePort` | Port on every node (auto-assigned for LoadBalancer/NodePort) |

```yaml
ports:
- port: 80          # clients call :80
  targetPort: 8080  # traffic lands on container :8080
  # nodePort: 32678 # auto-assigned by Kubernetes
```

#### Service Debugging Cheat Sheet

| Symptom | First thing to check |
|---|---|
| `Endpoints: <none>` | Selector mismatch — `kubectl get pods -l <selector>` |
| Traffic drops silently | Wrong `targetPort` — check container port vs targetPort |
| DNS not resolving | CoreDNS running? `kubectl get pods -n kube-system` |
| LoadBalancer `<pending>` forever | On minikube — expected. On EKS — check IAM/cloud-controller |
| Wrong pods receiving traffic | Selector too broad — tighten with `app.kubernetes.io/name` |

```bash
# Check endpoints (are pods being selected?)
kubectl get endpoints payment-api-internal -n finpay-system

# Check selector matches
kubectl get pods -n finpay-system -l app.kubernetes.io/name=payment-api

# DNS test
kubectl run test-dns --image=busybox:1.36 --rm -it --restart=Never \
  -- nslookup payment-api-internal.finpay-system.svc.cluster.local

# HTTP test
kubectl run test-curl --image=curlimages/curl:8.5.0 --rm -it \
  --restart=Never -n finpay-system \
  -- curl -s http://payment-api-internal/
```

#### Production Rules for Services

1. **Selector must be app-specific** — always include `app.kubernetes.io/name`
2. **ClusterIP for internal, LoadBalancer for external** — never create unnecessary ELBs
3. **NLB annotations are mandatory on EKS** — missing = Classic ELB or internet-facing
4. **`internal: true` for non-public services** — payment APIs must never be internet-facing
5. **`sessionAffinity: None` for stateless** — ClientIP only for stateful/WebSocket
6. **Always apply then verify** — never describe before applying
7. **Named ports** — use `name: http` not just `port: 80`
8. **Every Service gets labels** — needed for NetworkPolicies and cost allocation

---

## S02 — PODS — UPDATES (Extends existing S02)

### Probe Addition (from add.txt — verbatim)

- readinessProbe failing → Pod removed from Service Endpoints, but the container keeps running (Running vs Ready distinction).
- livenessProbe failing → kubelet kills and restarts the container — used to recover from deadlocks/hangs that a process can't self-recover from.
- startupProbe (a third type) protects slow-starting apps from being killed by livenessProbe before they've even finished booting.
- Static Pods are declared by a manifest file on the node's filesystem (kubelet watches a directory) rather than through the API server — kubeadm-based clusters use this to bootstrap the control plane itself (etcd, API server) before the API server exists to manage them via normal means. Not something typically hand-authored on EKS since AWS manages the control plane.
...
### Init Container — Frontend Waits for Backend (from add.txt — verbatim)

> Q5. Frontend and backend apps run in two different Pods. What configuration ensures the frontend starts only after the backend is up and running?

**Definition:** Kubernetes has no native 'wait for another Pod' scheduling primitive — the correct pattern is an initContainer on the frontend Pod that blocks until the backend is reachable, combined with a proper readiness probe on the backend.

**How It Works:**
- Add an `initContainers` entry to the frontend Pod spec that runs before the main container starts.
- The init container polls the backend Service (e.g. with a simple curl/wget loop or a wait-for-it script) until it responds successfully.
- Only once the init container exits successfully does Kubernetes start the main frontend container.
- The backend itself should have a `readinessProbe` so its Service endpoint only lists it once it's actually able to serve traffic, not just once the process has started.
...
initContainers:
  - name: wait-for-backend
    image: busybox:1.36
    command: ['sh', '-c',
      'until wget -q --spider http://backend-svc.cmg-backend.svc.cluster.local:8080/health; do echo waiting for backend; sleep 5; done']
```

---

## S04 — DEPLOYMENTS — UPDATES (Extends existing S04)

### Blue-Green Deployment — Verbatim Example (from add.txt)

- Deploy the new version as a fully separate Deployment (green) alongside the existing one (blue), each with a distinguishing label like `version: blue` / `version: green`.
- Both are fully up and health-checked independently before any traffic switch — green never receives production traffic during validation.
- The single production Service's selector is updated (e.g. `kubectl patch` or a GitOps commit) from `version: blue` to `version: green`.
- Because the Service selector change is nearly instant, traffic cuts over with no rolling window — and rollback is just as fast: patch the selector back to blue.

```yaml
# Service selector before cutover
selector:
  app: cmg-app
  version: blue

# cutover command
kubectl patch service cmg-app-svc -n cmg-app -p '{"spec":{"selector":{"version":"green"}}}'

# instant rollback
kubectl patch service cmg-app-svc -n cmg-app -p '{"spec":{"selector":{"version":"blue"}}}'
```

### Task3 Concepts — Verbatim (from task3.txt)

#### minReadySeconds

A Pod must remain Ready for the specified time before Kubernetes considers it Available.
...
Meaning: Pod becomes Ready → Wait 10 seconds → Deployment continues

**Purpose:** Avoids rolling forward immediately if a Pod becomes Ready but crashes shortly after.

**Interview Q:** Why use minReadySeconds? **A:** To ensure newly created Pods remain stable before Kubernetes continues the rollout.

---

#### revisionHistoryLimit

Controls how many previous ReplicaSets Kubernetes keeps.

```yaml
revisionHistoryLimit: 3
```

**Interview Q:** Why not keep unlimited ReplicaSets? **A:** To avoid unnecessary resource usage while still supporting rollback.

---

### Annotations Are Not Just for Documentation (from task3.txt — verbatim mistake)

#### MISTAKE — Thinking Annotations Are Only for Reading Purpose

**What I Thought:** I assumed annotations are just metadata used for documentation and have no impact on Kubernetes objects.

**Why This Is Wrong:** Annotations are **not used for selecting objects**, but they **can change the behavior of Kubernetes controllers or external systems**.
...
This annotation instructs AWS to create a Network Load Balancer. Without it — Service still exists, Pods still run, Labels still work — but AWS creates a different Load Balancer or uses its default behavior.

**Correct Understanding:**
- Labels → Used for Selection
- Annotations → Used for Configuration / Instructions

**Memory Trick:**
- Labels → Who am I?
- Annotations → What should be done with me?

---

## S20 — CLUSTER UPGRADE — UPDATES (Extends existing S20)

### EKS Control Plane Explained (from add.txt — verbatim)

**What is the EKS Control Plane?**

The EKS Control Plane is the managed part of Kubernetes that AWS runs for you. It includes the API Server, etcd, Scheduler, and Controller Manager. You don't manage these servers — AWS handles their availability, scaling, patching, and upgrades.
...
### Full EKS Upgrade Step-by-Step (from add.txt — verbatim)
...
# AWS Console / CLI / Terraform — AWS upgrades while keeping API available

# Step 4: Upgrade EKS Add-ons
# CoreDNS, kube-proxy, VPC CNI
kubectl get pods -n kube-system

# Step 5: Upgrade Worker Nodes
# Create new node group with new version / update managed node group
# New nodes join → Old nodes still running

# Step 6: Drain Old Nodes
kubectl drain <node-name> --ignore-daemonsets
# Kubernetes reschedules Pods to new nodes
...
# After confirming all workloads healthy, delete old node group
```

**How downtime is avoided:**
- Multiple replicas of each application
- Rolling updates move Pods gradually
- Readiness probes ensure traffic goes only to healthy Pods
- PodDisruptionBudgets prevent too many Pods unavailable during node draining
- EKS control plane remains available because AWS manages the upgrade

---

## S17 — HIGH AVAILABILITY — UPDATES (Extends existing S17)

### PDB — Full YAML Examples (from add.txt — verbatim)

A PodDisruptionBudget (PDB) protects your application during voluntary disruptions, such as node draining, cluster upgrades, or maintenance. It ensures that a minimum number of Pods stay available.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: nginx
        image: nginx:1.29.1    # ✅ pinned version
```

**How it works with 3 Pods + minAvailable:2:**
- ✅ Kubernetes can evict 1 Pod.
- ❌ It will not evict a second Pod until another Pod becomes available.
...
metadata:
  name: nginx-pdb
spec:
  maxUnavailable: 1      # Kubernetes can make only 1 Pod unavailable at a time
  selector:
    matchLabels:
      app: nginx
```
...
## S18 — SCALING — UPDATES (Extends existing S18)

### HPA Full Example (from add.txt — verbatim)

**What is Horizontal Pod Autoscaler (HPA)?**

Horizontal Pod Autoscaler (HPA) automatically increases or decreases the number of Pods in a Deployment, ReplicaSet, or StatefulSet based on metrics such as CPU, memory, or custom metrics.
...
- The Deployment should have CPU requests defined:
...
## S38 — NEW SECTION: NodeRestriction Admission Plugin (from noderestriction_txt.txt)

### What is NodeRestriction?

The **NodeRestriction Admission Plugin** is a Kubernetes security feature that prevents a **kubelet** (running on a node) from making unauthorized changes to its own Node object.

- Mainly used to stop a compromised node from pretending to be another type of node by modifying sensitive labels.
- Cluster administrators can still add/modify any node labels — NodeRestriction only restricts **kubelets**.

### Why NodeRestriction is Needed
...
Each kubelet regularly updates: Node status, CPU/Memory info, Ready/NotReady status, Running Pods.

**Security Problem:** Without protection, a compromised Node01 kubelet could add `gpu=true` label even without GPU → Kubernetes schedules GPU workloads to wrong node.
...
Any label beginning with this prefix is **protected**.

```
# Examples of protected labels:
node-restriction.kubernetes.io/pci=true
node-restriction.kubernetes.io/secure=true
...
```
Before NodeRestriction:
Node01 → Add Protected Label → API Server → ✔ Accepted

After NodeRestriction:
Node01 → Add Protected Label → NodeRestriction → ❌ Denied/Forbidden
```

### How to Enable NodeRestriction

```bash
# Edit the API Server manifest (static Pod)
...
# After (add NodeRestriction line):
spec:
  containers:
  - command:
    - kube-apiserver
...
**Why API Server restarts automatically:** It runs as a Static Pod. kubelet watches `/etc/kubernetes/manifests/`. Editing the YAML → kubelet notices → stops old Pod → starts new Pod. No manual restart needed.
...
### Testing NodeRestriction

```bash
# Step 1: SSH into node
ssh node01

# Step 2: Switch to kubelet identity (CRITICAL)
export KUBECONFIG=/etc/kubernetes/kubelet.conf
# Now every kubectl command is sent as the node01 kubelet
# Admin has unrestricted permissions — won't test NodeRestriction

# Step 3: Test protected label (should FAIL)
kubectl label node node01 node-restriction.kubernetes.io/two=123
# Expected: Error from server (Forbidden): nodes "node01" is forbidden

# Step 4: Test normal label (should SUCCEED)
kubectl label node node01 test/two=123
# Expected: node/node01 labeled

# Step 5: Test another node (should FAIL — different reason)
kubectl label node controlplane killercoda/two=123
# Expected: Forbidden — kubelet can only modify ITS OWN node
```
...
|---|---|---|
| `kubectl label node node01 node-restriction.kubernetes.io/two=123` | ❌ Forbidden | Protected prefix blocked by NodeRestriction |
| `kubectl label node controlplane killercoda/two=123` | ❌ Forbidden | Kubelet cannot modify another node (Node authorizer) |
| `kubectl label node node01 test/two=123` | ✅ Success | Normal label, not protected |
| Existing `node-restriction.kubernetes.io/*` labels | ✅ Remain | NodeRestriction blocks future changes, does NOT remove existing |

### What kubelet CAN and CANNOT update
...
Banking application should only run on secure nodes:

```yaml
# Protected label (only admin can set this)
kubectl label node node01 node-restriction.kubernetes.io/secure=true

# Pod uses this label for scheduling
nodeSelector:
  node-restriction.kubernetes.io/secure: "true"
```

If a hacker compromises a node → cannot add this protected label → Banking Pods remain on trusted nodes only. ✅
...
# Add protected label (works only if NodeRestriction disabled)
kubectl label node node01 node-restriction.kubernetes.io/one=123

# Remove existing protected label (admin only)
kubectl label node node01 node-restriction.kubernetes.io/one-
```

### Interview Questions — NodeRestriction

**Q1. What is the NodeRestriction Admission Plugin?**
A security admission plugin that prevents kubelets from modifying protected Node labels (`node-restriction.kubernetes.io/*`) and certain sensitive Node fields.

**Q2. Why is NodeRestriction required?**
To prevent a compromised node from pretending to be a trusted or specialized node by modifying sensitive labels.

**Q3. Which labels are protected?**
Labels starting with `node-restriction.kubernetes.io/`

**Q4. What happens if NodeRestriction is disabled?**
The kubelet can modify protected labels, allowing a compromised node to attract sensitive workloads.

**Q5. Does NodeRestriction block all node updates?**
No. It only blocks unauthorized changes to protected labels. Normal status updates (CPU, memory, Ready condition) continue to work.

**Common Mistake:** ❌ Thinking NodeRestriction prevents administrators from labeling nodes. Wrong — it only restricts kubelets, not cluster administrators.

**One-line Summary:** NodeRestriction is a Kubernetes security admission plugin that prevents kubelets from modifying protected node labels (`node-restriction.kubernetes.io/*`), ensuring compromised nodes cannot impersonate trusted nodes.

---

## S39 — CONTAINER HARDENING (from ContainerHardening_txt.txt)
...
Minimal images: smaller size, fewer vulnerabilities, faster downloads.
...
# ❌ Bad
USER root
# or no USER instruction (defaults to root)
...
If attacker compromises container → root → can modify files → can escape (in some cases) → can attack host.

**3. Remove Unnecessary Packages**

```dockerfile
# ❌ Don't install unless required
...
Never install: curl, wget, vim, nano, ssh, gcc, make, git (unless required).
...
Production image contains only the application — NOT Go compiler, Git, build cache.
...
# Output: HIGH/CRITICAL — openssl, glibc
# Then rebuild with patched versions

# Other tools: Clair, Grype, Docker Scout
```
...
securityContext:
  readOnlyRootFilesystem: true
# Attacker cannot: install malware, write scripts, modify binaries
```
...
Linux capabilities like NET_ADMIN, SYS_ADMIN, SYS_MODULE — drop everything not needed.

**10. Prevent Privileged Containers**

```yaml
# ❌ Never
securityContext:
  privileged: true
# Privileged container ≈ Host root access
...
**11. Run with Non-root Security Context**
...
resources:
  ...
  limits:
    cpu: "500m"
    memory: "512Mi"
# Prevents: DoS, memory exhaustion, CPU abuse
```

**13. Sign Images**

Tools: Cosign, Notary — ensures image hasn't been tampered with.

**14. Keep Images Updated**

Old image with OpenSSL vulnerability → Rebuild → Deploy latest patched image.

**15. Don't Install SSH**

Containers should run one main process, not behave like virtual machines. Never install: ssh, systemd, cron.

### Complete Kubernetes Security Context (Hardened)
...
### Practical Task: Harden a Dockerfile

**Requirements:**
1. Use specific version `ubuntu:20.04`
2. Remove layer caching issues with apt-get
3. Remove hardcoded secret — pass as runtime env variable `TOKEN`
4. Make it impossible to exec into container using bash

```dockerfile
# Solution — Final hardened Dockerfile
FROM ubuntu:20.04
...
```bash
# Build and run with secret as runtime env var
podman build -t app .
podman run -e TOKEN=2e064aad-3a90-4cde-ad86-16fad1f8943e app

# Test bash exec is blocked
podman run -d -e TOKEN=2e064aad-3a90-4cde-ad86-16fad1f8943e app sleep 1d
podman exec -it <id> bash   # ❌ fails — bash removed
podman exec -it <id> sh     # ✅ works — sh still available
```

**Why each step:**
- `ubuntu:20.04` (not `latest`) → reproducible, no surprise version changes
- `apt-get update && apt-get install` (one RUN line) → prevents stale layer cache
- `ENV TOKEN` not in Dockerfile → secret only at runtime, never baked into image
- `RUN rm /usr/bin/bash` → prevents bash exec (scratch image is even better)

---

## SCHEDULING UPDATES (Extends S14, from task3.txt — verbatim concepts)

### Weight in preferredDuringScheduling

```yaml
weight: 100
```

- How strongly Kubernetes should prefer this scheduling rule.
- Range: 1–100. Higher = higher scheduling preference.
- Weight only works with `preferredDuringSchedulingIgnoredDuringExecution` — NOT with `requiredDuringScheduling`.

**Interview answer:** Weight defines the priority of a preferred scheduling rule.

### maxSkew Explained (from task3.txt — verbatim)

```yaml
maxSkew: 1
```

- Maximum allowed difference in the number of Pods between topology domains.

```
# Allowed (maxSkew:1):
Zone A = 3, Zone B = 2, Zone C = 2 → difference = 1 ✅

# Not Allowed (maxSkew:1):
Zone A = 5, Zone B = 2, Zone C = 2 → difference = 3 ❌
```

**Interview answer:** maxSkew controls how evenly Pods are distributed across nodes or Availability Zones.

### AWS NLB Annotations (from task3.txt)
...
Purpose: Create AWS Network Load Balancer and register Pod IPs directly.

**Interview:** Why use NLB annotations? **A:** To instruct AWS how to create and configure the Load Balancer.

### Session Affinity (from task3.txt — verbatim)
...
- Client continues reaching the same Pod for 10800 seconds = 3 hours.
- Useful for: Banking, Shopping Cart, Legacy Applications.

**Interview Q:** What does timeoutSeconds mean? **A:** Duration for which Kubernetes keeps a client's IP mapped to the same Pod.

### Pod IP Commands (from task3.txt)
...
## QUICK REVISION (from add.txt — verbatim)

| Concept | One-liner |
|---|---|
| Service | Stable IP/DNS for a dynamic set of Pods, load-balanced by kube-proxy |
| kube-proxy | Programs iptables/IPVS on each node to route Service traffic to Pods |
| CRD | Extends the K8s API with a custom resource type; needs a controller to do anything |
| CNI | Plugin spec for Pod networking/IP assignment (AWS VPC CNI on EKS) |
| Cross-namespace DNS | svc-name.namespace.svc.cluster.local — always safe to use |
| Startup ordering | No native cross-Deployment dependency — use initContainers |
| Blue-Green cutover | Two full Deployments, flip Service selector for instant switch |
| Running vs Ready | Running = process started; Ready = passing readiness, eligible for traffic |
| Node traffic issue triage | Node → Pod readiness → Endpoints → CNI/kube-proxy on that node |

### Frequently Forgotten (from add.txt — verbatim)

- A Pod showing 'Running' can still have 0/1 Ready and receive zero traffic — always check the READY column.
- Service selector changes are the fastest possible rollback mechanism in Kubernetes — faster than redeploying.
- EKS VPC CNI IP exhaustion looks like a scheduling problem ('Insufficient pods') but is actually a networking limit.
- initContainers must exit 0 to let the main container start — an infinite retry loop with no timeout will hang the Pod forever in Init state.

---

## UPDATES TO MASTER RAPID FIRE — New Entries

> Appended to Master Rapid Fire chapter. Deduplicated against existing entries.

### 🔥 Networking — New Additions

| Q | A |
|---|---|
| ContainerCreating stuck = IP exhaustion? | Check: kubectl describe pod → "failed to assign IP" → check ENI limits, subnet capacity |
| Fix IP exhaustion on EKS? | Add nodes, expand subnet /24→/20, enable prefix delegation, use larger instance |
| EKS aws-node DaemonSet restart? | kubectl rollout restart daemonset aws-node -n kube-system (affects networking temporarily) |
| t3.medium max pods (VPC CNI)? | ~17 (3 ENIs × 6 IPs per ENI) |
| 502 error after deploy = ? | Pod Running but not Ready — missing/misconfigured readinessProbe |
| 503 error after deploy = ? | No healthy targets at all — check ALB target group health |
| Cross-namespace DNS format? | svc-name.namespace.svc.cluster.local |
| Within-namespace DNS? | Short name (svc-name) — DNS search domains fill in the rest |

### 🔥 Security — New Additions

| Q | A |
|---|---|
| NodeRestriction plugin purpose? | Prevents kubelets from modifying protected node labels (node-restriction.kubernetes.io/*) |
| How to enable NodeRestriction? | Add --enable-admission-plugins=NodeRestriction to kube-apiserver manifest |
| NodeRestriction restricts admins? | NO — only restricts kubelets, not cluster administrators |
| Protected label prefix? | node-restriction.kubernetes.io/ |
| NodeRestriction removes existing labels? | NO — only blocks future changes |
| kubelet can modify other nodes? | NO — Node authorizer restricts to own node only |
| Test NodeRestriction? | export KUBECONFIG=/etc/kubernetes/kubelet.conf then kubectl label node node01 node-restriction.kubernetes.io/x=1 |
| Minimal base image for Java? | gcr.io/distroless/java17 |
| Remove bash to prevent exec? | RUN rm /usr/bin/bash in Dockerfile |
| Layer caching fix in Dockerfile? | Combine: RUN apt-get update && apt-get install -y <pkg> (single RUN) |
| Never store secrets in? | Docker image (ENV or COPY of credentials) — use K8s Secrets or external managers |

### 🔥 Services — New Additions

| Q | A |
|---|---|
| Too-broad selector causes? | Silent misrouting — traffic goes to unrelated pods with zero error |
| Best primary selector label? | app.kubernetes.io/name: <app> — specific to one app |
| NLB annotation for EKS? | service.beta.kubernetes.io/aws-load-balancer-type: "nlb" |
| Keep NLB internal? | service.beta.kubernetes.io/aws-load-balancer-internal: "true" |
| sessionAffinity: None + config block? | Contradiction — config block only valid with ClientIP |
| sessionAffinity for REST API? | None (stateless) — ClientIP only for WebSocket/stateful |
| Apply then verify order? | Always apply FIRST, then verify AFTER |
| Classic ELB deprecated? | YES — always use NLB for new EKS workloads |
| Endpoints empty = ? | Selector mismatch (90% of cases) or readinessProbe failing |

### 🔥 Workloads — New Additions

| Q | A |
|---|---|
| Frontend wait for backend pattern? | initContainer polling backend health endpoint until exit 0 |
| Blue-Green fastest rollback? | kubectl patch svc — selector change back to blue — instant |
| Weight range in scheduling? | 1–100. Higher = stronger preference |
| Weight only works with? | preferredDuringSchedulingIgnoredDuringExecution (not required) |
| maxSkew: 1 means? | Max 1 Pod difference between any two topology zones |

---

## PROCESSING SUMMARY

**Files processed:** add.txt, noderestriction_txt.txt, ContainerHardening_txt.txt, task3.txt, task3-mistakes-and-concepts.md

**New concepts added:**
- ContainerCreating/IP exhaustion troubleshooting (full 8-step framework)
- NodeRestriction Admission Plugin (complete new section S38)
- Container Hardening Checklist + practical Dockerfile task (S39 extended)
- Service selector mistakes (6 verbatim mistakes from production)
- Blue-Green verbatim example with patch commands
- 502/503 error post-deploy troubleshooting
- Frontend→Backend init container pattern
- PDB YAML examples (minAvailable + maxUnavailable)
- Session affinity deep dive (correct vs incorrect usage)
- EKS Control Plane explanation + full upgrade steps
- Probe clarification (Running vs Ready distinction)
- CoreDNS cross-namespace DNS verbatim

**Existing concepts updated:**
- S05 Services (NLB annotations, debugging cheat sheet, port vs targetPort table)
- S06 Networking (kube-proxy internals, CNI deep dive)
- S17 HA (PDB with maxUnavailable variant)
- S18 Scaling (HPA YAML with prerequisites)
- S20 Cluster Upgrade (EKS Control Plane + step-by-step)

**Duplicate questions skipped:**
- "What is a Service?" — covered in S05
- "What is HPA?" — covered in S18
- "What is PDB?" — covered in S17
- "Rolling update internals" — covered in S04
- "kubectl rollout undo" — covered in S04 + Cheat Sheet
- "readinessProbe vs livenessProbe basics" — covered in S02
- "Deployment vs StatefulSet vs DaemonSet" — covered in S04/S11/S12

**Handbook sections modified:** S02, S04, S05, S06, S14, S17, S18, S20, S38 (new), S39 (extended)

**Master Rapid Fire entries added:** 30 new Q→A entries (networking, security, services, workloads)


---

# UPDATES — MERGED FROM FAST-KUBERNETES REFERENCE (July 2025)

> **Duplicate check completed:** 48 concepts already fully covered in S01–S50.
> Only 13 genuinely new items added below, placed under correct sections.
> Source: Fast-Kubernetes repo (omerbsezer/Fast-Kubernetes) + Kubernetes Commands Cheatsheet

---

## S23 — CHEAT SHEET UPDATES (Extends existing S23)

### Complete kubectl Command Reference — New Additions

> **[DUPLICATE CHECK]:** Basic pod, deployment, rollout, service, scale commands already in S23.
> Adding only commands NOT yet in handbook.

```bash
# ─── kubectl explain (NEW — not previously in handbook) ──────────────
kubectl explain pod                    # show pod object schema + fields
kubectl explain deployment             # show deployment object schema
kubectl explain pods.spec.containers   # drill into specific field
kubectl explain serviceaccount         # explain SA object

# ─── Label imperative commands (NEW) ────────────────────────────────
kubectl label pods pod1 team=development        # add label to pod
kubectl label pods pod1 team-                   # remove label (append -)
kubectl label --overwrite pods pod1 team=test   # overwrite existing label
kubectl label pods --all foo=bar                # add label to ALL pods
kubectl label nodes minikube hddtype=ssd        # label a node (for nodeSelector)
kubectl label nodes minikube hddtype-           # remove label from node

# ─── Annotation imperative commands (NEW) ───────────────────────────
kubectl annotate pods annotationpod foo=bar     # add annotation
kubectl annotate pods annotationpod foo-        # remove annotation
...
kubectl expose deployment payment --type=LoadBalancer --name=payment-lb

# ─── Describe (NEW verbose form) ────────────────────────────────────
kubectl describe pods firstpod                  # full pod details incl. Events
kubectl describe deployment payment-service     # deployment details
kubectl describe nodes worker-1                 # node details incl. taints, conditions
kubectl describe endpoints payment-service      # show all Pod IPs behind service

# ─── Logs (extended forms — NEW) ────────────────────────────────────
kubectl logs -f firstpod                        # -f = follow/tail live log
kubectl logs firstpod -c sidecarcontainer       # specific container in multi-container pod
kubectl logs -f multicontainer -c sidecarcontainer  # follow specific container
kubectl logs --since=1h payment-pod             # logs from last 1 hour
kubectl logs --tail=50 payment-pod              # last 50 lines only

# ─── Selector filtering (NEW) ───────────────────────────────────────
...
kubectl get pods -l "app=firstapp,tier!=frontend" --show-labels   # NOT equal
kubectl get pods -l "!app" --show-labels                          # does NOT have key
kubectl get pods -l "app notin (firstapp)" --show-labels          # NOT IN

# ─── Namespace switching (NEW) ──────────────────────────────────────
kubectl config set-context --current --namespace=development   # change default ns
kubectl config set-context --current --namespace=default        # back to default
kubectl delete namespaces development

# ─── Port-forward extended forms (NEW) ──────────────────────────────
kubectl port-forward pod/mypod :5000                           # random local port
kubectl port-forward --address 0.0.0.0 pod/mypod 8888:5000    # all interfaces
kubectl port-forward deployment/mydeployment 5000 6000
kubectl port-forward service/myservice 5000 6000

# ─── Multi-container exec (NEW) ─────────────────────────────────────
kubectl exec -it multicontainer -c webcontainer -- /bin/sh     # -c = container name
kubectl exec -it multicontainer -c sidecarcontainer -- /bin/sh
kubectl logs -f multicontainer -c sidecarcontainer

# ─── Minikube dashboard (NEW) ───────────────────────────────────────
minikube addons enable dashboard
minikube addons enable metrics-server
minikube dashboard
# WSL/WSL2: sensible-browser http://127.0.0.1:PORT/api/v1/namespaces/kubernetes-dashboard/...
```

---

## S13 — JOBS & CRONJOBS UPDATES (Extends existing S13)

### Cron Schedule Reference (NEW — not in handbook)

> **[DUPLICATE CHECK]:** Job parallelism/completions/backoffLimit already in S13. Adding cron syntax reference only.
...
# Examples:
#   5 * * * *     = At minute 5 of every hour: 00:05, 01:05, 02:05...
#   */5 * * * *   = Every 5 minutes: 00:00, 00:05, 00:10...
#   0 */2 * * *   = Every 2 hours at :00 — 00:00, 02:00, 04:00...
#   0 2 * * *     = Daily at 2 AM
...
## S09 — STORAGE UPDATES (Extends existing S09)

### PV Reclaim Policy — Recycle (NEW)

> **[DUPLICATE CHECK]:** Retain and Delete covered in S09. Adding Recycle for completeness.

```yaml
# persistentVolumeReclaimPolicy: Recycle
# - Volume NOT deleted after PVC release
# - ALL DATA inside volume IS deleted (wiped)
# - Volume becomes Available again for new PVC binding
# - DEPRECATED in newer K8s versions — do NOT use for new workloads
# - Use Retain (production) or Delete (dev/test) instead
persistentVolumeReclaimPolicy: Recycle
```

| Policy | Volume | Data | Status After | Use |
|---|---|---|---|---|
| Retain | Kept | Preserved | Released (manual cleanup) | ✅ Production |
| Delete | Deleted | Deleted | Gone | Dev/test |
| Recycle | Kept | **Wiped** | Available again | ❌ Deprecated |

### Secret from-file Imperative Commands (NEW)

> **[DUPLICATE CHECK]:** `--from-literal` covered in S15. Adding `--from-file` forms.

```bash
# Create secret from file (hides password from shell history)
...
# Create secret from JSON config file
kubectl create secret generic mysecret4 --from-file=config.json

# Why use --from-file instead of --from-literal?
# --from-literal=password=mypass → password visible in shell history (ps aux, history)
# --from-file=password=pass.txt  → password read from file, not visible in history
```

---

## S14 — SCHEDULING UPDATES (Extends existing S14)

### Taint — PreferNoSchedule Effect (NEW)

> **[DUPLICATE CHECK]:** NoSchedule and NoExecute covered in S14. Adding PreferNoSchedule.

```bash
# Three taint effects:
kubectl taint node worker-1 app=production:NoSchedule
# NoSchedule: Pod CANNOT be scheduled on this node if no matching toleration
# Existing pods: NOT affected

kubectl taint node worker-1 app=production:NoExecute
# NoExecute: Pod CANNOT run on node. Existing pods WITHOUT toleration are EVICTED.
# Existing pods WITH toleration: stay (can add tolerationSeconds for grace period)

kubectl taint node worker-1 app=production:PreferNoSchedule
# PreferNoSchedule: Scheduler TRIES to avoid placing pod here
# BUT if no other node is available, pod CAN still be scheduled here
# Softest taint effect — best-effort avoidance only

# Taint imperative commands
kubectl taint node minikube platform=production:NoSchedule    # add taint
kubectl taint node minikube platform-                         # remove taint (append -)
kubectl describe nodes minikube | grep Taints                 # view taints
```

| Effect | New Pods | Existing Pods |
|---|---|---|
| NoSchedule | ❌ Cannot schedule | ✅ Not affected |
| PreferNoSchedule | ⚠️ Avoid if possible | ✅ Not affected |
| NoExecute | ❌ Cannot schedule | ❌ Evicted if no toleration |

---

## S11 — STATEFULSET UPDATES (Extends existing S11)

### StatefulSet — Key Differences Summary (NEW detailed table)

> **[DUPLICATE CHECK]:** StatefulSet vs Deployment table already in S11. Adding lifecycle detail.

```
StatefulSet Pod Naming:
  statefulsetName-0    ← created first, must be Running before -1 starts
  statefulsetName-1    ← created second
  statefulsetName-2    ← created third

Scale UP:  0 → 1 → 2 (sequential, wait for each)
Scale DOWN: 2 → 1 → 0 (reverse order, graceful)

PVC per Pod:
  statefulsetName-0 → PVC volumeName-statefulsetName-0
  statefulsetName-1 → PVC volumeName-statefulsetName-1
  statefulsetName-2 → PVC volumeName-statefulsetName-2

Scale down to 0 replicas: PVCs REMAIN (data preserved)
Scale back up: PVCs REATTACH to same pod name automatically
```

---

## S00 — KUBERNETES DASHBOARD (NEW SECTION)

> **[NEW TOPIC — not previously in handbook]**

**WHAT:** Kubernetes Dashboard is a web-based UI for managing and monitoring cluster resources.

**WHY:** kubectl is CLI-only. Dashboard provides visual overview of all workloads, resources, and status at a glance without writing commands.

**HOW:** Deployed as a Pod in kubernetes-dashboard namespace. Access via kubectl proxy or minikube addon.

### What You Can See in Dashboard

- All Workloads: Deployments, ReplicaSets, Pods, StatefulSets, DaemonSets, Jobs, CronJobs
- Memory and CPU usage per workload
- Services, Endpoints, Ingresses
- PersistentVolumes, PersistentVolumeClaims
- ConfigMaps, Secrets, StorageClasses
- RBAC: ClusterRoles, Roles, RoleBindings, ServiceAccounts
- Namespaces, Network Policies, Nodes
...
# Open dashboard (opens browser automatically)
minikube dashboard

# WSL/WSL2 — open browser manually
kubectl proxy
# then open: http://127.0.0.1:8001/api/v1/namespaces/kubernetes-dashboard/services/http:kubernetes-dashboard:/proxy/
```
...
> ⚠️ **Never expose dashboard externally without authentication.** Default setup is localhost only via kubectl proxy. Do NOT create a LoadBalancer/NodePort Service for dashboard in production — this exposes cluster management to the internet.

---

## S00 — PLAY WITH KUBERNETES (NEW)

> **[NEW — quick reference for free online K8s practice]**

**URL:** https://labs.play-with-k8s.com/

- Free browser-based K8s playground — no installation needed
- 4-hour sessions, multiple nodes available
- Perfect for practicing kubeadm cluster setup, kubectl commands, K8s concepts

```bash
# Quick kubeadm cluster on Play With Kubernetes (1 master + 2 workers):
...
# On each WORKER node (token from kubeadm init output):
...
## UPDATES TO MASTER RAPID FIRE — New Entries from Fast-Kubernetes

> Adding only entries not already in Master Rapid Fire. Duplicates skipped.

### 🔥 Commands — New Additions

| Q | A |
|---|---|
| kubectl explain purpose? | Shows object schema + all available fields (kubectl explain pod.spec.containers) |
| Label a node? | kubectl label nodes node-1 hddtype=ssd |
| Remove a label? | kubectl label pods pod1 team- (append minus sign) |
| Overwrite a label? | kubectl label --overwrite pods pod1 team=test |
| Label all pods? | kubectl label pods --all foo=bar |
| Expose deployment as Service? | kubectl expose deployment backend --type=ClusterIP --name=backend |
| Follow logs live? | kubectl logs -f <pod> |
| Logs for specific container? | kubectl logs <pod> -c <container-name> |
| Port-forward to random port? | kubectl port-forward pod/mypod :5000 (OS picks port) |
| Exec into specific container? | kubectl exec -it <pod> -c <container-name> -- /bin/sh |
| Annotate a pod? | kubectl annotate pods mypod key=value |
| Remove annotation? | kubectl annotate pods mypod key- |
| Change default namespace? | kubectl config set-context --current --namespace=myns |

### 🔥 Scheduling — New Additions

| Q | A |
|---|---|
| PreferNoSchedule effect? | Soft avoid — scheduler tries not to use node but will if no other option |
| NoSchedule vs PreferNoSchedule? | NoSchedule = hard block. PreferNoSchedule = soft preference. |
| NoExecute evicts existing pods? | YES — existing pods without matching toleration are evicted |
| Remove a taint? | kubectl taint node node-1 key- (append minus to key) |

### 🔥 Storage — New Additions

| Q | A |
|---|---|
| Reclaim policy Recycle? | Deprecated — keeps volume, wipes all data, returns to Available |
| --from-file secret benefit? | Password not visible in shell history (vs --from-literal) |
| StatefulSet scale down order? | Reverse: pod-2 deleted before pod-1 before pod-0 |
| PVCs on StatefulSet scale-to-0? | PVCs REMAIN — data preserved, reattach on scale-up |

---

## PROCESSING SUMMARY — Fast-Kubernetes Merge

**Source:** Fast-Kubernetes repo (omerbsezer/Fast-Kubernetes) + Kubernetes Commands Cheatsheet

**Duplicate check:** 48 concepts already fully covered → skipped

**New concepts added:**
- kubectl explain command reference
- Label/annotation imperative commands (kubectl label pods/nodes, kubectl annotate)
- kubectl expose deployment
- kubectl logs -f (follow) + -c (container) + --since + --tail
- Port-forward extended forms (random port, all-addresses, deployment/service)
- Multi-container exec (-c flag)
- Cron schedule syntax reference + crontab.guru link
- PV Reclaim Policy: Recycle (deprecated — added for completeness)
- Secret --from-file (vs --from-literal — shell history security)
- Taint PreferNoSchedule effect + complete taint effects table
- StatefulSet scale up/down order + PVC persistence detail
- Kubernetes Dashboard (new section S00 — minikube + real cluster setup)
- Play With Kubernetes (free online lab)

**Sections modified:** S09, S11, S13, S14, S23 (new commands), S00 (Dashboard + PWK)

**Duplicate questions skipped:** 48 (parallelism, completions, backoffLimit, kubeadm, port-forward basics, emptydir, hostpath, ClusterIP, NodePort, StatefulSet basics, PVC, RBAC, Deployment, rollout, all networking basics, all security basics)
