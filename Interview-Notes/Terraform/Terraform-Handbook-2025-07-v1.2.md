...
> **V1 — ASG & Launch Template Mistakes Integrated**
>
> ✅ Processed a 10-category production-mistakes document (Categories
> 1–9 already covered — 3 gaps filled: object-interpolated-as-string
> in §12.1, default-SG-unmanaged in §12.3, bastion/app public-IP swap
> in §10) plus Category 10 (ASG + Launch Template — entirely new) and a
> narrative "Task 2" doc on Auto Scaling fundamentals. Result: new
> §16 (Auto Scaling Groups & Launch Templates, 11 subsections) and
> §17 (ASG mistakes table), new §12.6 (Process & Engineering
> Discipline, 6 items), a tag-accuracy note in §5.3, and 8 new cheat
> sheet rows. Full summary at the end of §17.
...
| 16 | Auto Scaling Groups & Launch Templates — 11 subsections |
| 17 | ASG & Launch Template Mistakes Table |
...
| 39 | AWS Provider | ✅ | §7–§11, §16 (ASG/Launch Templates) |
| 40 | Azure Provider | ⬜ | — |
| 41 | GCP Provider | ⬜ | — |
...
> **⚠️ Common Mistake — Copy-Pasted Wrong Tag Values**
>
> A common bug: private data subnets end up tagged `Name = "public-1"`
> because the block was copy-pasted from the public subnet section and
> the `Name` value was never updated. Always verify a resource's `Name`
> tag actually reflects what the resource is — don't just trust the
> copy-paste.
...
| Public IP     | Bastion private / App public (reversed) | Bastion needs associate_public_ip_address = true; app tier = false |
...
| 13     | Object interpolated as string | "eip-\${each.value}" (whole for_each object) | "eip-\${each.key}" or "eip-\${each.value.attribute}" |
...
| 7      | Default SG left unmanaged  | Any ENI accidentally attached gets open access | aws_default_security_group with NO ingress/egress blocks — absence of blocks = absence of rules |
...
**12.6 Process & Engineering Discipline**

|        |                                                          |                                                                                                       |
|--------|------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|
| **\#** | **Mistake**                                              | **Fix**                                                                                                |
| 1      | Skipping `terraform fmt` + `terraform validate` before commit/review | Always run both — 10 seconds catches ~80% of beginner mistakes before a human reviews the code       |
| 2      | Fixing a bug in one place, missing the same bug class elsewhere | When you find a bug CLASS (e.g. object interpolated as string), search the WHOLE file for every other occurrence and fix all instances in one pass |
| 3      | Reverting already-fixed code by pasting from an older draft | One file, one source of truth — never paste from older drafts; build forward from the last working version; use Git to track every change |
| 4      | Hardcoded secrets in a variable's `default` value        | No default for secrets — `sensitive = true`, supply the real value via a gitignored `terraform.tfvars` |
| 5      | New folder (new state) per feature/task instead of extending existing state | One folder = one state = one environment — add resources to it, don't restart it. Separate state PER LAYER (vpc/compute/data) is valid, but layers talk via `terraform_remote_state` (§2.8), never by recreating existing resources |
| 6      | Deleting/moving a resource still referenced elsewhere, causing "Reference to undeclared resource" | Before deleting or moving any resource, grep the whole project for every reference to that resource name — confirm zero references remain first |
...
| health_check_type = "ELB"                               | Always set behind an ALB — EC2 default misses app failures |
| propagate_at_launch = true                               | Required on ASG tags — else launched EC2s get no tags      |
| templatefile() vs file()                                 | templatefile substitutes variables — file reads as-is      |
| key_name (not public_key) on Launch Template             | key_name = key pair NAME, not RSA content                  |
| Launch Template \> Launch Configuration                 | Always use Launch Template — versioned, IMDSv2, current gen |
| AL2023: dnf, not yum                                     | yum is Amazon Linux 2 only                                 |
| min ≤ desired \< max                                     | desired = max blocks scale-out permanently                 |
...
### SECTION 16 [ V1 COMPUTE ] — AUTO SCALING GROUPS & LAUNCH TEMPLATES


**16.1 Launch Template vs Launch Configuration**

|                 |                                                        |                        |
|-----------------|----------------------------------------------------------|--------------------------|
| **Aspect**      | **Launch Template**                                     | **Launch Configuration** |
| Status          | Current, recommended                                    | Legacy — deprecated       |
| Versioning      | Yes — multiple versions, roll back to any version        | No versioning             |
| Feature support | IMDSv2, T2/T3 Unlimited, mixed instance types, newest EC2 features | Missing newer features |
| Use in new work | Always                                                   | Never                     |

**16.2 AMI Data Source — Naming Rule**

- `data "aws_ami" "amazon_linux" {}` — the block type `aws_ami` is
  fixed by the provider and cannot be renamed; the local name
  (`amazon_linux`) is yours to choose freely.
- Amazon Linux 2023 and Amazon Linux 2 are the current supported
  versions — there is no "Amazon Linux 2025." Always confirm the real
  current release name rather than guessing a year.
- Never hardcode an AMI ID — always fetch dynamically via `data
  "aws_ami"` with `owners = ["amazon"]` (see §10.1).

**16.3 Launch Template Key Config**
...
| key_name = var.key_name \# the KEY PAIR NAME — not RSA content       |
| metadata_options {                                                    |
| http_endpoint = "enabled" \# turns the metadata service ON            |
| http_tokens = "required" \# enforces IMDSv2 — no IMDSv1 fallback     |
| http_put_response_hop_limit = 1 \# limits metadata access to 1 network hop |
| }                                                                    |
| user_data = base64encode(templatefile("\${path.module}/user_data.sh.tpl", { app_env = var.environment })) |
| lifecycle { create_before_destroy = true }                            |
| }                                                                    |

> **⚠️ Common Mistake — key_name Wrong Value Type**
>
> `key_name` on a launch template expects the AWS **key pair NAME**
> (e.g. `"my-key"`), not the RSA public key content
> (`"ssh-rsa AAAA..."`). `aws_key_pair.key_name` = name.
> `aws_key_pair.public_key` = RSA content — don't mix them up.

**Why IMDS (Instance Metadata Service) exists**

- An internal API inside every EC2 instance, reachable only from that
  instance, that applications use to self-discover: instance ID,
  private IP, IAM role credentials, region, and availability zone.
- SSRF attack against IMDSv1: a vulnerable app is tricked into
  `GET http://169.254.169.254/latest/meta-data/iam/security-credentials/`
  and the attacker steals the instance's IAM credentials.
- IMDSv2 blocks this: it requires a `PUT` request first to obtain a
  session token — SSRF (which typically can only trigger a GET) cannot
  complete that handshake, so the credential theft path is closed.

**16.4 ASG Core Configuration**
...
| desired_capacity = 3                                            |
| max_size = 6                                                    |
| health_check_type = "ELB" \# NOT the "EC2" default — see 16.5   |
| vpc_zone_identifier = values(var.private_subnet_ids)            |
| launch_template { id = aws_launch_template.app.id; version = "$Latest" } |
| tag {                                                            |
| key = "Name"                                                    |
| value = "cmg-app"                                                |
| propagate_at_launch = true \# without this, launched EC2s get NO tags |
| }                                                                |
| }                                                                |

- **Desired / Min / Max**: Desired = the current number of instances
  the ASG actively tries to maintain. Min = floor. Max = ceiling. Rule:
  `min ≤ desired ≤ max`.

> **❌ Real Mistake — desired_capacity Equal to max_size**
>
> `min_size=1, desired_capacity=6, max_size=6` — the ASG is already at
> its ceiling, so the scale-out policy can never fire even under heavy
> load. Always leave headroom: e.g. `desired=3, max=6`.

**16.5 Health Check Types**

|                        |                                          |
|------------------------|--------------------------------------------|
| **health_check_type**  | **What It Verifies**                       |
| EC2 (default)          | The instance is running — nothing about the app |
| ELB (production)       | The ALB target health check passes — the actual application is responding |

- An app can crash while the underlying EC2 instance is still
  "healthy" by EC2's own check — that's exactly the gap `ELB` health
  checks close. Always set `health_check_type = "ELB"` when the ASG
  sits behind an ALB.

**16.6 Scaling Policies & Cooldown**

|                     |                                                     |
|---------------------|-------------------------------------------------------|
| **Policy Type**     | **Behavior**                                          |
| Simple Scaling      | Fixed change in capacity (e.g. +1 or -1 instance)     |
| Step Scaling        | Increase/decrease amount varies by how far past the threshold the metric is |
| Target Tracking     | Automatically maintains a target metric value (e.g. keep average CPU at 50%) — the easiest to operate, adjusts itself |

- `ChangeInCapacity` (Simple Scaling) launches or terminates a fixed
  number of instances per alarm trigger — e.g. CPU > 70% → +1 instance,
  CPU < 30% → -1 instance.
- **Cooldown**: a wait period after a scaling action before another
  one can trigger. Without it: CPU high → launch → still high (new
  instance not warmed up yet) → launch again → again — runaway
  over-scaling. With cooldown: launch → wait → re-check → scale only if
  still needed.

**16.7 templatefile() vs file()**

|                    |                                              |
|--------------------|--------------------------------------------------|
| **Function**       | **Behavior**                                     |
| file()             | Reads a file as-is — static, no substitution     |
| templatefile()     | Reads a file AND substitutes `${}` variables inside it — dynamic |
...
**16.8 create_before_destroy for Compute Resources**

- Without it: Terraform destroys the old resource FIRST, then creates
  the new one — a gap with zero capacity in between, causing downtime.
- With it: the new resource is created and traffic-ready, THEN the old
  one is destroyed — no gap.
- Apply this to: Launch Templates, Security Groups, and IAM resources
  that get replaced — anywhere a brief overlap is safer than a brief
  gap. See also §5.2 and the `replace_triggered_by` production example.
- **instance_refresh**: rolls a Launch Template change out to
  currently-running ASG instances automatically (without it, only NEW
  instances get the updated template — existing ones are untouched
  until they're replaced some other way).
- **update_default_version**: keep the ASG referencing the latest
  Launch Template version rather than pinning to a specific version
  number, so new launches always pick up the newest config.

**16.9 AL2023 Uses dnf, Not yum**

|                                                    |
|------------------------------------------------------|
| \# WRONG — yum is for Amazon Linux 2                  |
| sudo yum install -y nginx                             |
|                                                        |
| \# CORRECT — AL2023 uses dnf                           |
| sudo dnf install -y nginx                              |

**16.10 Module Design for Compute**

> **❌ Real Mistake — VPC Created Inside an ASG Module**
>
> An ASG/compute module created its own `aws_vpc` resource internally.
> One module = one responsibility: the VPC module owns networking, the
> ASG module owns compute — pass `private_subnet_ids` in as an input
> variable, never create foundational networking inside a compute
> module. See also §12.4.

- Declared-but-unused variables (never referenced anywhere in the
  module's `.tf` files, never passed from root) should be deleted — if
  you can't find `var.X` used in `main.tf`, remove the variable.

**16.11 Production Troubleshooting Checklist**

1. ASG **Activity History** — shows exactly why a scaling action
   succeeded or failed.
2. **Launch Template** — confirm the AMI, instance type, and key name
   are all valid.
3. **Subnets** — confirm `vpc_zone_identifier` subnets exist and have
   IP capacity.
4. **Health Checks** — confirm `health_check_type` and the ALB target
   group health check settings agree with each other.
5. **IAM** — confirm the instance profile has the permissions the
   application needs.
6. **Quotas** — check EC2 instance limit and ENI limits for the
   account/region.
7. **CloudWatch** — check the metric and alarm driving the scaling
   policy actually reflects real load.

- Always troubleshoot from the AWS Console first (Activity History)
  before diving into Terraform state.

### SECTION 17 [ V1 REFERENCE ] — ASG & LAUNCH TEMPLATE MISTAKES TABLE


|        |                                                    |                                                       |
|--------|------------------------------------------------------|-----------------------------------------------------------|
| **\#** | **Mistake**                                        | **Fix**                                                   |
| 1      | Creating foundational resources (VPC) inside a compute module | One module = one responsibility — pass subnet IDs in, never create VPC in an ASG module |
| 2      | key_name set to RSA key content instead of key pair name | key_name = var.key_name (the NAME); public_key argument is for the RSA content |
| 3      | desired_capacity equal to max_size                | Leave headroom — min ≤ desired < max so scale-out can fire |
| 4      | Missing owners on aws_ami data source              | owners = ["amazon"] — restricts to trusted AMIs only, not the whole community |
| 5      | health_check_type left at default "EC2" behind an ALB | Set health_check_type = "ELB" — catches app-level failures EC2 checks miss |
| 6      | Missing propagate_at_launch on ASG tags            | propagate_at_launch = true — without it, launched EC2s get no tags |
| 7      | yum used on Amazon Linux 2023                      | AL2023 uses dnf, not yum                                  |
| 8      | Declared module variables never referenced anywhere | If var.X isn't used in main.tf, delete the variable       |

---

## ✅ Processing Summary — ASG/Launch Template Mistakes Upload

**Items processed:** 10 mistake categories (60 individual mistakes,
Categories 1–10) + 1 narrative doc (11 "Mistake" write-ups on Auto
Scaling fundamentals) + 1 ASG/Launch Template quick-reference table.

**New concepts added (13):** §16.1 Launch Template vs Configuration ·
§16.2 AMI naming rule · §16.3 Launch Template key config + IMDSv2/SSRF
mechanics + why IMDS exists · §16.4 ASG core config (min/desired/max)
· §16.5 Health check types · §16.6 Scaling policies + cooldown ·
§16.7 templatefile() vs file() · §16.8 create_before_destroy for
compute + instance_refresh + update_default_version · §16.9 AL2023
dnf · §16.10 Compute module design rule · §16.11 Troubleshooting
checklist · §17 ASG mistakes table · §12.6 Process & Engineering
Discipline (6 items).

**Existing concepts updated (4):** §12.1 (+1 row: object-as-string
interpolation) · §12.3 (+1 row: unmanaged default SG) · §10 (+1 row:
bastion/app public IP) · §5.3 (+ tag-accuracy caution).

**Duplicate questions/mistakes skipped (~52):** Categories 1, 2, 3,
4 (minus the default-SG gap), 5, 6, 7 (minus the copy-paste-tag gap),
8 (minus the bastion/app-IP gap), and most of 9 — all already fully
covered by §1, §5, §9, §10, §12.1–§12.4, and §15.13 from earlier
passes; only the genuine gaps were added rather than re-stating
already-covered mistakes.

**Handbook sections modified:** §5.3, §10, §12.1, §12.3, new §12.6,
new §16, new §17, TOC, Coverage Tracker, changelog.

**Rapid Fire entries:** still deferred — per-topic 🔥 Rapid Fire blocks
and the 🔥🔥 Master Rapid Fire chapter remain queued as the next pass,
now covering §1–§17.
