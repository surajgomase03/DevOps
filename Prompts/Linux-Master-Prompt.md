...
Example: `Linux-Handbook-2026-07-v1.md` — after July ends, this file
is archived forever.

## Rule 2 – New Month = New Handbook
A new file (e.g. `Linux-Handbook-2026-08-v2.md`) is created only on
...
- Linux history, GNU/Linux, distributions
...
- Process management
- Package management (rpm, yum, dnf, apt, zypper)
- systemd & journald
- Networking
- SSH
- DNS
...
- Shell scripting
- Cron & systemd timers
- Security (SELinux/AppArmor/PAM/sudo)
- Backup & recovery
- Containers (Namespaces, cgroups, Podman, Docker basics)
- SLES
- RHEL
- Ubuntu
- SAP HANA on Linux
...
- Master Rapid Fire (all topics) — always last
...
- Consolidates (not duplicates) the per-topic Rapid Fire Q&A from
  every topic in the active file, in the coverage order above
...
- Simple English. Explain every technical term.
...
a short cross-reference note goes in the Linux/AWS/Terraform handbook.
...
- Expand existing topics with new information (regenerate the whole
  topic, per Rule 4/6).
...
- Verify rapid-fire entries are deduplicated and merged into the
  Master Rapid Fire chapter
...
(`Linux-Handbook-YYYY-MM-vX.md`) that is a complete Linux
Administrator, DevOps, SRE and SAP HANA Linux reference — sufficient
for senior (5–10 years) production roles and interview preparation
without needing additional Linux study material.
